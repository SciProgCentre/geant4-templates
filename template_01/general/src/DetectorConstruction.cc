...
DetectorConstruction::DetectorConstruction(ServerSettings* serverSettings) : G4VUserDetectorConstruction(), serverSettings(serverSettings) {
    parser.Read(serverSettings->gdml, false);
    logger = Logger::instance();
    logger->print("GDML file " + serverSettings->gdml + " read");
...
void DetectorConstruction::setFieldFactory(IFieldFactory *fieldFactory_) {
    this->fieldFactory = fieldFactory_;
}
